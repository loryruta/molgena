...
class MolGraph(object):
    ...
    def __init__(self, smiles):
        self.smiles = smiles
        self.mol = get_mol(smiles)  # Parse the molecule SMILES to rdkit molecule
    ...
    def find_clusters(self):
        """ Breaks the molecule into clusters.
        Clusters are a combination of SSSR (Smallest Set of Smallest Rings) and bonds not in rings.
        All atoms will eventually be part of a cluster.
        """
        mol = self.mol
        ...
        # Make root be part of the first cluster
        ...
        # Assign to every atom a list of indices to the clusters it belongs
    ...
    def tree_decomp(self):
        """ Having split the molecule into clusters, creates a Graph of the molecule using them.
        This is the so-called Junction Tree. """

        clusters = self.clusters
        graph = nx.empty_graph(len(clusters))
        for atom, nei_cls in enumerate(self.atom_cls):
            if len(nei_cls) <= 1:  # The atom is in just one cluster (= it is the cluster!)
                continue

            # How many bonds does this atom make?
            bonds = [c for c in nei_cls if len(clusters[c]) == 2]

            # How many (> 4 long) rings does this atom make?
            rings = [c for c in nei_cls if len(clusters[c]) > 4]  # need to change to 2

            if len(nei_cls) > 2 and len(bonds) >= 2:
                # This atom is in many clusters and make many bonds; should be a cluster itself
                ...
                # This atom is in many rings; should be a cluster
    ...
    def label_tree(self):
        def dfs(order, pa, prev_sib, x, fa):  # Depth-First Search
            pa[x] = fa
        ...
        for atom in mol.GetAtoms():
            atom.SetAtomMapNum(atom.GetIdx() + 1)  # Annotate the atoms with a sequential number

        tree = self.mol_tree
        for i, cluster in enumerate(self.clusters):
            inter_atoms = set(cluster) & set(self.clusters[pa[i]]) if pa[i] >= 0 else set([0])
            cmol, inter_label = get_inter_label(mol, cluster, inter_atoms)  # TODO EXCEPTION WITH ZINC!
            ...
            tree.nodes[i]['label'] = (smiles, ismiles) if len(cluster) > 1 else (smiles, smiles)
            tree.nodes[i]['cluster'] = cluster
            ...
            if pa[i] >= 0 and len(self.clusters[pa[i]]) > 2:  # uncertainty occurs in assembly
                ...
                tree.nodes[i]['assm_cands'] = get_assm_cands(mol, hist, inter_label, pa_cls, len(inter_atoms))  # TODO EXCEPTION WITH ZINC!

                child_order = tree[i][pa[i]]['label']
                diff = set(cluster) - set(pa_cls)
    ...
    def build_mol_graph(self):
        """ Given the molecule, creates a labeled Directed Graph.
        Labeled because both nodes and edges are labeled with resp. node and edge information.
        Directed because a bond has a direction (begin atom and to atom).
        """
        mol = self.mol
